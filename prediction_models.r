...
PredictAndDrawAll = function(DATA, filename = "tmp", ...) {
  allPredictions = matrix(c(0), nrow = 9, ncol=length(DATA[[2]][,"party_changed"]))
  ...
  allPredictions[4,] = treePredictionLessThresholdToSplit(DATA[[1]], DATA[[2]])
  allPredictions[5,] = tresholdPrediction(DATA[[1]], DATA[[2]], "own_dst")
  allPredictions[6,] = tresholdPrediction(DATA[[1]], DATA[[2]], "own_dst_norm")
  allPredictions[7,] = tresholdPrediction(DATA[[1]], DATA[[2]], "own_mode_dst")
  allPredictions[8,] = tresholdPrediction(DATA[[1]], DATA[[2]], "avg_streak_against_party")
  allPredictions[9,] = tresholdPrediction(DATA[[1]], DATA[[2]], "max_streak_against_party")
